...
var o = d.querySelector('#action_arrow').onclick = attachCircleMarker;
...
function attachCircleMarker(options) {
	var start, end;
	...
	var onMUP = function(options) {		
		if (!start) {
			start = {top: circleMarker.get('top'), left: circleMarker.get('left')};
			line.set({'x1': start.left});
			line.set({'y1': start.top});
			line.set({'x2': start.left});
			line.set({'y2': start.top});
			canvas.add(line);
